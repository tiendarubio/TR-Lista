document.addEventListener('DOMContentLoaded', async () => {
  ...
  const btnHistHoy = $('btnHistHoy');
  const btnHistModo = $('btnHistModo');
  ...
  let histMode = 'edit'; // 'edit' = modo normal, 'hist' = solo lectura (historial)
  ...
  function setHistMode(mode) {
    histMode = mode;
    if (histMode === 'hist') {
      btnHistModo.textContent = 'Modo: historial';
      btnHistModo.classList.remove('btn-outline-primary');
      btnHistModo.classList.add('btn-primary');
      // Desactivar edición
      ...
      btnHistModo.textContent = 'Modo: edición';
      btnHistModo.classList.remove('btn-primary');
      btnHistModo.classList.add('btn-outline-primary');
  ...
  function renderEditableState(enabled) {
    const inputs = body.querySelectorAll('input.qty');
    const btnToggles = body.querySelectorAll('.btn-toggle');
    const btnMove = body.querySelectorAll('.btn-move-alterna');
    const btnDelete = body.querySelectorAll('.btn-delete-row');

    inputs.forEach(i => { i.disabled = !enabled; });
    btnToggles.forEach(b => { b.disabled = !enabled; });
    btnMove.forEach(b => { b.disabled = !enabled; });
    btnDelete.forEach(b => { b.disabled = !enabled; });
  }

  async function loadStoreStateFrom(dateString = null) {
    body.innerHTML = '';
    ...
    if (histMode === 'hist') {
      renderEditableState(false);
    } else {
      renderEditableState(true);
    }
  }
  ...
  // Historial: fecha por defecto = hoy
  if (histDate) {
    histDate.value = getTodayString();
  }

  if (btnHistHoy) {
    btnHistHoy.addEventListener('click', async () => {
      if (!histDate) return;
      const today = getTodayString();
      histDate.value = today;
      currentHistDate = today;
      setHistMode('hist');
      await loadStoreStateFrom(today);
    });
  }

  if (btnHistModo) {
    btnHistModo.addEventListener('click', async () => {
      if (!histDate) return;
      if (histMode === 'edit') {
        // Cambiar a modo historial si hay fecha seleccionada
        const d = histDate.value || getTodayString();
        histDate.value = d;
        currentHistDate = d;
        setHistMode('hist');
        await loadStoreStateFrom(d);
      } else {
        // Regresar a modo edición (estado actual)
        setHistMode('edit');
        await loadStoreState();
      }
    });
  ...
  if (histDate) {
    histDate.addEventListener('change', async () => {
      if (!histDate.value) return;
      currentHistDate = histDate.value;
      setHistMode('hist');
      await loadStoreStateFrom(currentHistDate);
    });
  }
  ...
  searchInput.addEventListener('input', () => {
    if (histMode === 'hist') return; // no buscar en modo historial
  ...
  async function moveRowToAlterna(tr) {
    try {
      ...
      // Guardar lista destino (actualiza estado actual y historial de hoy)
  ...
  btnSave.addEventListener('click', async () => {
    if (histMode === 'hist') {
      Swal.fire('Modo historial', 'No puedes guardar mientras estás viendo el historial. Cambia a modo edición.', 'info');
      return;
    }
    const binId = getBinId(storeSelect.value, versionSelect.value);
    const payload = collectPayload();
    try {
      const result = await saveToBin(binId, payload);
      lastUpdateISO = result.meta.updatedAt;
      lastSaved.innerHTML =
        '<i class="fa-solid fa-clock-rotate-left me-1"></i>' +
        'Última actualización: ' + formatSV(lastUpdateISO);
      Swal.fire('Guardado', 'Checklist guardado correctamente en Firestore (incluyendo historial del día).', 'success');
    } catch (e) {
      console.error(e);
  ...
  btnClear.addEventListener('click', () => {
    if (histMode === 'hist') {
      Swal.fire('Modo historial', 'No puedes limpiar mientras estás viendo el historial. Cambia a modo edición.', 'info');
      return;
    }
      ...
      if (res.isConfirmed) {
        ...
        try {
          ...
          lastSaved.innerHTML =
            '<i class="fa-solid fa-clock-rotate-left me-1"></i>' +
            'Última actualización: ' + formatSV(lastUpdateISO);
          Swal.fire('Listo', 'Checklist vacío guardado en Firestore.', 'success');
        } catch (e) {
          console.error(e);
          Swal.fire('Error', String(e), 'error');
        }
