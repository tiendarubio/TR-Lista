document.addEventListener('DOMContentLoaded', async () => {
  ...
  async function moveRowToAlterna(tr) {
    try {
      // No permitir mover en modo histórico
      ...
      if (!toDoc) {
        await Swal.fire('Configuración incompleta', 'No hay documento configurado para la lista destino de esta tienda.', 'error');
        return;
      }
  ...
  btnClear.addEventListener('click', async () => {
    ...
    Swal.fire({
      ...
      if (res.isConfirmed) {
        ...
        await saveChecklistToFirestore(docId, payload, today);
